...
def set_config(config={}, link={}, node={}, margin={}, **kwargs):
    ...
    config["link"] = {
        **{"color": "source-target", "stroke_opacity": 0.5, "color_static": "#D3D3D3"},
        **link,
    }
    config["node"] = {
        **{"align": "justify", "width": 15, "padding": 15, "color": "currentColor"},
        **node,
    }
...
def write_html(X, config, logger=None):
    ...
    try:
        ...
        jinja_env = Environment(
            loader=PackageLoader(package_name="d3blocks.sankey", package_path="d3js")
        )
...
def get_data_ready_for_d3(df, labels):
    ...
    list_id = np.array(
        list(map(lambda x: labels.get(x)["id"], df["source"]))
        + list(map(lambda x: labels.get(x)["id"], df["target"]))
    )
    list_name = np.array(
        list(map(lambda x: labels.get(x)["label"], df["source"]))
        + list(map(lambda x: labels.get(x)["label"], df["target"]))
    )
